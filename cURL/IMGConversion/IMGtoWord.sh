...
# 2. Create Task
curl --location --request GET 'https://api-server.compdf.com/server/v1/task/img/docx?language=1' \
--header 'Authorization: Bearer accessToken'

# 3. Upload File
curl --location --request POST 'https://api-server.compdf.com/server/v1/file/upload' \
--header 'Authorization: Bearer accessToken' \
--form 'file=@"/input_files/test.png"' \
--form 'taskId="taskId"' \
--form 'password=""' \
--form 'parameter="{   \"wordLayoutMode\":1, \"isAllowOcr\":1, \"isContainOcrBg\":0, \"isOnlyAiTable\":0  }"' \
--form 'language=""'
